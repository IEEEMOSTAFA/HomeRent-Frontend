...
// import { useAdminAnalytics, useUnverifiedOwners, useVerifyOwner } from "@/hooks/admin/useAdminApi";
...
//                   onClick={pulseCard}
//                   disabled={isPending}
//                   className={`w-full gap-2 text-xs font-semibold h-9 transition-all duration-200 ${owner.verified
...
//                     disabled={isPending}
//                     className={`h-9 text-sm font-semibold ${owner.verified
...
//     <button
//       onClick={onClick}
//       className={`px-3 py-1.5 text-xs font-medium rounded-full border transition-colors duration-150 ${active
...
export default function OwnerVerificationPage() {
  ...
  // ✅ analytics থেকে real DB counts নেওয়া হচ্ছে
  const { data: analytics } = useAdminAnalytics();
  ...
  return (
    <TooltipProvider>
      <div className="space-y-7 p-1">
        ...
        {/* ── Stats ── */}
        {/* ✅ FIX: analytics থেকে real DB count নেওয়া হচ্ছে */}
        {/* ❌ আগে: list.length, verifiedCount, pendingCount — শুধু current page এর data */}
        {/* ✅ এখন: analytics.totalOwners, verifiedOwners, unverifiedOwners — সব DB এর সত্যিকারের count */}
        {!isLoading && (
          <div className="grid grid-cols-3 gap-3">
            <StatCard
              icon={<Users size={16} />}
            ...
            <StatCard
              ...
              label="Verified"
              value={analytics?.verifiedOwners ?? 0}
            ...
            <StatCard
              ...
              label="Pending"
              value={analytics?.unverifiedOwners ?? 0}
              color="bg-amber-500"
              delay={200}
            />
          </div>
        ...
        {/* ── Footer ── */}
        {/* ✅ FIX: total count analytics থেকে নেওয়া হচ্ছে */}
        {!isLoading && (analytics?.totalOwners ?? list.length) > 0 && (
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }} className="text-xs text-muted-foreground/60 text-center pb-2">
            Showing {filtered.length} of {analytics?.totalOwners ?? list.length} owners
          </motion.p>
        )}
