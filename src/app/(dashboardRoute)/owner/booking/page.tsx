"use client";
// src/app/(dashboardRoute)/owner/booking/page.tsx
    ...
    <div className="space-y-6">
      ...
      {!isLoading && error && (
        <div className="text-center py-20">
          <p className="text-red-500 text-sm">ডেটা লোড করতে সমস্যা হয়েছে। পেজ রিফ্রেশ করুন।</p>
        </div>
      )}
            ...
            <Card key={b.id} className="shadow-none hover:shadow-sm transition-shadow">
              <CardContent className="p-5">
                <div className="flex gap-4">
                  ...
                  {/* Thumbnail — ✅ FIXED: images?.[0] দিয়ে safe access */}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap gap-x-4 gap-y-1 mt-2.5 text-xs text-muted-foreground">
                      <span className="flex items-center gap-1.5">
                        <User size={11} /> {b.user?.name ?? "Unknown"}
                      </span>
                      <span className="flex items-center gap-1.5">
                        <Calendar size={11} /> {new Date(b.moveInDate).toLocaleDateString()}
                      </span>
                      <span className="flex items-center gap-1.5">
                        <Users2 size={11} /> {b.numberOfTenants} tenant(s)
                      </span>
