...
// ─── Social Links with proper icons from react-icons ───────────────────────────
const SOCIAL_LINKS = [
  { 
    icon: FaFacebook, 
  ...
  { 
    icon: FaYoutube, 
    href: "https://www.youtube.com/@BackendMostafa", 
    label: "YouTube",
...
// ─── Social Icon Button ────────────────────────────────────────────────────────
function SocialBtn({ icon: Icon, href, label, color }: { icon: React.ElementType; href: string; label: string; color?: string }) {
  return (
    <motion.a
      href={href}
      target="_blank"
      ...
      whileHover={{ scale: 1.1, y: -2 }}
      whileTap={{ scale: 0.92 }}
      className={`w-9 h-9 rounded-lg border border-slate-700 bg-slate-800/50 flex items-center justify-center transition-all duration-300 group ${color}`}
    >
      <Icon className="w-4 h-4 text-slate-400 group-hover:text-current transition-colors" />
    </motion.a>
  );
}
      ...
      <div ref={ref} className="relative z-10 mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <motion.div variants={fadeLeft} className="lg:col-span-2">
            {/* Logo */}
            <Link href="/" className="inline-flex items-center gap-3 mb-5 group">
              <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-emerald-500 to-teal-500 flex items-center justify-center shadow-lg shadow-emerald-500/20 group-hover:shadow-emerald-500/40 transition-shadow duration-300 overflow-hidden">
                <Image
            ...
            {/* Contact - Updated with original information */}
            <div className="space-y-2.5 mb-6">
              <div className="flex items-center gap-3 text-sm text-slate-500">
                ...
                >
                  almostafa.cu@gmail.com
                </a>
              </div>
              <div className="flex items-center gap-3 text-sm text-slate-500">
                <Phone className="w-3.5 h-3.5 text-emerald-500 shrink-0" />
                <a 
                  href="tel:+8801889271256" 
                  className="hover:text-emerald-400 transition-colors"
                >
                  {CONTACT_INFO.phone}
                </a>
            ...
            {/* Socials - Updated with proper icons from react-icons */}
            <div className="flex items-center gap-2.5">
              {SOCIAL_LINKS.map((s) => (
                <SocialBtn key={s.label} {...s} />
              ))}
