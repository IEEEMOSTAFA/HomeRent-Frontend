...
// ─── Status metadata ──────────────────────────────────────────────────────────
const STATUS_META: Record<string, { pill: string; dot: string; glow: string }> = {
  CONFIRMED:       { pill: "bg-emerald-500/15 text-emerald-400 border-emerald-500/30", dot: "bg-emerald-400", glow: "shadow-emerald-500/20" },
  PENDING:         { pill: "bg-amber-500/15 text-amber-400 border-amber-500/30",       dot: "bg-amber-400",   glow: "shadow-amber-500/20"   },
  PAYMENT_PENDING: { pill: "bg-blue-500/15 text-blue-400 border-blue-500/30",          dot: "bg-blue-400",    glow: "shadow-blue-500/20"    },
  ACCEPTED:        { pill: "bg-violet-500/15 text-violet-400 border-violet-500/30",    dot: "bg-violet-400",  glow: "shadow-violet-500/20"  },
  DECLINED:        { pill: "bg-red-500/15 text-red-400 border-red-500/30",             dot: "bg-red-400",     glow: "shadow-red-500/20"     },
  CANCELLED:       { pill: "bg-slate-500/15 text-slate-400 border-slate-500/30",       dot: "bg-slate-500",   glow: ""                      },
};
const getMeta = (s: string) => STATUS_META[s] ?? STATUS_META["PENDING"];

// ─── Status Icon renderer (keeps forwardRef components OUT of module-scope objects)
function StatusIcon({ status, size = 12 }: { status: string; size?: number }) {
  switch (status) {
    case "CONFIRMED":
    case "ACCEPTED":        return <CheckCircle2 size={size} />;
    case "PENDING":         return <AlertCircle  size={size} />;
    case "PAYMENT_PENDING": return <Clock        size={size} />;
    case "DECLINED":
        ...
        >
          ...
          <div className="relative h-40 overflow-hidden bg-white/5">
            ...
            <div className="absolute top-3 right-3">
              ...
              >
                <StatusIcon status={booking.status} /> {booking.status.replace("_"," ")}
              </motion.div>
            </div>
...
// ─── Scroll Progress Bar (separate component — hooks can't be called inside IIFE) ──
